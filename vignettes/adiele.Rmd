...
#country <- "nig"
# weather data
wth <- derive_wth_vars(readRDS(file.path(lcp, "ex/weather.rds")))
...
get_wth <- function(name, year) {
  w <- wth[(wth$NAME==name) & (wth$YEAR %in% (year + 0:1)), ]
...
for(year in c(2016, 2017)) {
  yr <- substr(year, 3, 4)
#	wdata <- get_weather_old(system.file("weather", package="LINTCAS"), country="nig", station='1', year=year, endtime = 506)
...
```{r fig5}
#Get the data
MEASURED_DATA <- read.csv(file.path(lcp, "ex/Biomass_NPKuptake_plantpart_totals_per_treatment.csv"))
