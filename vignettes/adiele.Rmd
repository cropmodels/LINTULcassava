...
for(year in c(2016, 2017)) {
  ...
  cntr <- list(timestep=1)
  Pot_Edo <- LC_run(wth, crop, soil, c(mng, IRRIGF=TRUE), cntr)
	mod[[paste0("Pot_Edo", yr)]] <- data.frame(Location="Edo", Pot_Edo)
	  
	Wlim_Edo <- LC_run(wth, crop, soil, c(mng, IRRIGF=FALSE), cntr)
  ...
  mng <- list(year=year, starttime = soil$DOYPL-100, endtime = soil$DOYHAR)
	...
	Pot_CRS <- LC_run(wth, crop, soil, c(mng, IRRIGF=TRUE), cntr)
	mod[[paste0("Pot_CRS", yr)]] <- data.frame(Location="CRS", Pot_CRS)
	  
	Wlim_CRS <- LC_run(wth, crop, soil, c(mng, IRRIGF=FALSE), cntr)
  ...
  mng <- list(year=year, starttime = soil$DOYPL-100, endtime = soil$DOYHAR)
	...
	Pot_Ben <- LC_run(wth, crop, soil, c(mng, IRRIGF=TRUE), cntr)
	mod[[paste0("Pot_Ben", yr)]] <- data.frame(Location="Benue", Pot_Ben)
	  
	Wlim_Ben <- LC_run(wth, crop, soil, c(mng, IRRIGF=FALSE), cntr)
...
DAP_vs_IPAR <- function(Series1=TRT1,Series2=NULL,Series3=NULL, LOCYR, LEG=TRUE,
  ...
  plot(Series1$DAP, Series1$fPAR, type="l", 
        xlab = "Days after planting", ylab = "IPAR, f",
        ylim = c(0,1), xlim=c(0,450), lwd = 1.5,
  ...
  if(length(Series3)>1){
      lines(Series3$DAP, Series3$fPAR, col=blue, lwd=1.5, lty=4)
      ...
      points(Series3$DAP, Series3$fPAR, col = blue, pch = 19)
...
Figure2 <- function(DATA){
  one_plot <- function(loc, year, leg=FALSE) {
    locyr <- DATA$Location==loc & DATA$Year==year 
    i <- which(locyr & DATA$Treatment=="NfPfK1")
    j <- which(locyr & DATA$Treatment=="NfPfKf")
    k <- which(locyr & DATA$Treatment=="NfPfKfMn")
    DAP_vs_IPAR(Series1=DATA[i,],Series2=DATA[j,],Series3=DATA[k,], 
                LOCYR=paste(loc, year, collapse=", "),
                LEG=leg, LEGTEXT=c('NfPfK240', 'NfPfKf', 'NfPfKfMN'))
  }

  par(mfrow=c(2,2), oma=c(3,3,1,1), mar=c(1.5,1.5,0,0))
  one_plot("Edo", 2016, TRUE)
  one_plot("Benue", 2016)
  one_plot("Edo", 2017)
  one_plot("Benue", 2017)
